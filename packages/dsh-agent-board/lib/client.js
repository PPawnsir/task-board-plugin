...
function apply(ctx) {
    ...
    function DetailView() {
      ...
      return React.createElement('div', { style: { padding: '4px 2px' } },
        ...
        task.escalation ? React.createElement('div', { style: { marginBottom: 8, padding: '8px 10px', border: '1px solid ' + C.err, borderRadius: 6, background: 'color-mix(in srgb, ' + C.err + ' 8%, transparent)' } },
          ...
          React.createElement('button', { onClick: submitArbitration, disabled: !arbAnswer.trim(), style: { fontSize: 11, padding: '4px 12px', border: 'none', borderRadius: 4, background: C.err, color: C_INV, cursor: 'pointer', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 4 } }, ic('scale', 12), '提交裁决')) : null,
    ...
    function TopPanel() {
      ...
      if (view === 'dashboard') { content = React.createElement(Dashboard) }
        ...
        content = React.createElement('div', null, React.createElement(FilterBar, null), hasFilter && active.length === 0 && archived.length === 0 ? React.createElement('div', { style: { textAlign: 'center', padding: 16, color: C.text2, fontSize: 11 } }, '无匹配任务') : null, React.createElement('div', { style: { fontSize: 10, color: C.text2, marginBottom: 6 } }, state.selectMode ? '多选模式：点击卡片勾选，底部批量操作' : '提示：拖拽卡片到目标列即可流转状态'), React.createElement('div', { style: { display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 } }, cols), archived.length > 0 ? React.createElement('div', { style: { marginTop: 6, padding: '5px 8px', background: C.nested, borderRadius: 5, display: 'flex', alignItems: 'center', gap: 6 } }, ic('archive', 11), React.createElement('span', { style: { fontSize: 10, color: C.text2 } }, '已归档 ' + archived.length + ' 个任务'), React.createElement('button', { onClick: function () { state.view = 'archive'; notify(); fetchArchived() }, style: { fontSize: 9, padding: '2px 7px', border: '1px solid ' + C.brand, borderRadius: 3, background: 'transparent', color: C.brand, cursor: 'pointer' } }, '前往归档页 →')) : null, React.createElement(BatchBar, null))
      }
