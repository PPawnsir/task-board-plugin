...
- **Team 模式**：开启后主窗口的直接执行工具被硬拦截，所有工作走看板；Worker 歧义上报主窗口裁决
...
本包由 `scripts/build-pkg.cjs` 从动态插件源码（`host-v30.js` / `client-v30.js`）机械转换生成。改源码后重跑 `node scripts/build-pkg.cjs` 再重装。
