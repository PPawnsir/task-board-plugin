return {
  ...
  apply(ctx) {
    ...
    function rpc(method, args) { var a = args || {}; a.sessionId = state.sessionId; return host.call(method, a) }
    function fetchTasks() {
      ...
      rpc('get-tasks').then(function (d) {
        ...
        state.strictExec = !!(d && d.strictExec)
    ...
    // v64 执行约束开关（与 Team 模式解耦）：仅研发阶段开启，开启后主窗口 write/edit/pwsh 被硬拦截
    function StrictSwitch(props) { var on = props.on; function toggle() { rpc('set-strict-exec', { enabled: !on }).then(fetchTasks).catch(function () {}) } return React.createElement('button', { onClick: toggle, title: on ? '执行约束已开启：主窗口的直接编辑/执行被拦截，改动必须走看板（研发阶段用）' : '开启执行约束：拦截主窗口 write/edit/pwsh，强制改动走看板（调研阶段勿开）', style: { fontSize: 12, padding: '4px 10px', border: '1px solid ' + (on ? C.err : C.border), borderRadius: 8, cursor: 'pointer', fontWeight: 500, background: on ? C.err : 'transparent', color: on ? '#fff' : C.text2, transition: 'all .15s' } }, '🔒 约束' + (on ? ' ON' : '')) }
      ...
      return React.createElement('div', { style: { position: 'fixed', top: 44, left: (layL + 8) + 'px', right: (layR + 8) + 'px', maxHeight: '60vh', zIndex: 900, background: C.bg, border: '1px solid ' + C.border, borderRadius: '0 0 10px 10px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', overflow: 'hidden' } },
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px', borderBottom: '1px solid ' + C.border, flexShrink: 0, flexWrap: 'wrap', gap: 4 } },
          ...
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' } },
            ...
            React.createElement(StrictSwitch, { on: state.strictExec }),
